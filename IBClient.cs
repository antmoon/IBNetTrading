...
namespace IBSampleApp//直接拷贝了过来，引入时，需要using IBSampleAPP
{
    public class IBClient : EWrapper
    {
        ...
        public IBClient()//这里修改了默认构造函数，增加了一个signal，这样默认构造函数不需要参数了
        {
            signal = new EReaderMonitorSignal();
            clientSocket = new EClientSocket(this, signal);

            clientSocket.eConnect("127.0.0.1", 7497, 6);//上来就主动连接，那么策略类的连接都省掉了7496,7497,4001,4002
            //Create a reader to consume messages from the TWS. The EReader will consume the incoming messages and put them in a queue
            var reader = new EReader(clientSocket, signal);
            reader.Start();
            //Once the messages are in the queue, an additional thread need to fetch them

            Thread ps = new Thread(() => { while (clientSocket.IsConnected()) { signal.waitForSignal(); reader.processMsgs(); } });
