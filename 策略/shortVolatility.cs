...
{
    class shortVolatility:TradingInterface
    {
        ...
        public shortVolatility(IBClient ibclient) : base(ibclient)
        {
            fillEvent += onFill;
        }
        ...
        public override void TickOptionComputation(int tickerId, int field, double impliedVolatility, double delta, double optPrice, double pvDividend, double gamma, double vega, double theta, double undPrice)
        {
            ...
            if (field == 12)//成交价
            {
                if (!sellPlaced[curContract] && contractPosition[curContract].holding == 0)
                {
                    Order sellCall = new Order();
                    sellCall.Action = "SELL";
                    // buyCall.OrderType = "LMT";
                    sellCall.OrderType = "MKT";
                    sellCall.TotalQuantity = 1;
                    // buyCall.LmtPrice =Math.Round(optPrice,2);
                    sellCall.Transmit = true;                    
                    PlaceOrder(curContract, sellCall);
                    sellPlaced[curContract] = true;
                }
                else if (contractPosition[curContract].holding < 0)
                {
                    if (contractPosition[underlying].holding == 0)
                    {
                        ...
                        buyUnderlying.TotalQuantity = Math.Round(100 * callDelta, 0);
