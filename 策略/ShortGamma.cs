...
{
    class ShortGamma:TradingInterface
    {      
        ...
        public ShortGamma(IBClient ibclient) : base(ibclient)
        {
        ...
        public override void SecurityDefinitionOptionParameterEnd(int reqId)
        {
            ...
            ibclient.ClientSocket.reqMktData(reqId, contractIDrecord[reqId], "106", false, new List<TagValue>());//获取隐含波动率  
        ...
        {
            if (optionReady.ContainsKey(contract)&& optionReady[contract] == true && ImpliedVolatility[contract] != 0D)//option链和隐含波动率都有了
            {
              //  TimeSpan timeSpan = mostRecentExpireTime[contract].AddDays(1.5).Subtract(DateTime.Now);
                ...
                //double upperlimitPrice = contractPrice[contract].Last() * (1 + Standarddeviation);//strangle款跨式方法，straddle可以是直接和现价比
                //double bottomlimitPrice = contractPrice[contract].Last() * (1 - Standarddeviation);
                double upperlimitPrice = contractPrice[contract].Last() ;//straddle可以是直接和现价比
                double bottomlimitPrice = contractPrice[contract].Last() ;

                Console.WriteLine("ImpliedVolatility：" + ImpliedVolatility[contract]);
                ...
                Console.WriteLine(DateTime.Now + ",要交易的call行权价是：" + CallATMstrike[contract] + ",本策略目前交易的行权价是:" + callToTradeToday[contract].Strike);//调试的目的
                ...
                if (callToTradeToday[contract].Strike != CallATMstrike[contract] || callToTradeToday[contract].Strike==0D)//检查是否已经交易过，保证每个行权价只交易一次
                {
                    ...
                    if (!callAlreadyTraded[contract].Contains(CallATMstrike[contract]))
                    {
                ...
                Console.WriteLine(DateTime.Now+",要交易的put行权价是：" + PutATMstrike[contract] + ",本策略目前交易的行权价是:" + putToTradeToday[contract].Strike);//调试的目的
                ...
                ibclient.ClientSocket.cancelMktData(requestId[contract]);//只交易一次，如果要持续不断的交易，就去掉本行
        ...
        {
            if (USStockTime.AMESNow.DayOfWeek != DayOfWeek.Friday)//不是同一天，就收盘后自动平仓，赚THETA，是同一天，则盘后手动对冲即可
                base.TakeCareOfTime();
